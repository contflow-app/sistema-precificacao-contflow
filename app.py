...
with st.sidebar:
    ...
    percentual_taxas = st.slider("% Taxas Variáveis", 0.0, 30.0, 5.0, step=0.5, help="Percentual médio de taxas sobre cada venda.") / 100
    ...
    percentual_reserva = st.slider("% Reserva/Reinvestimento", 0.0, 50.0, 5.0, step=1.0, help="Meta de reserva/reinvestimento em relação ao faturamento.")
...
# --- EXIBIÇÃO DE METAS ---
st.metric("Margem de Contribuição Total Necessária", f"{mc_media_percentual*100:.1f}%")
st.info(f"""
**Metas Globais (Baseadas no Faturamento Previsto de R$ {faturamento_previsto:,.2f}):**
- Despesas Fixas: R$ {despesa_fixa_total:,.2f} ({percentual_custo_fixo*100:.1f}%)
- Lucro Líquido Alvo: R$ {valor_lucro:,.2f} ({percentual_lucro_desejado*100:.1f}%)
- Reserva/Reinvestimento: R$ {valor_reserva:,.2f} ({percentual_reserva*100:.1f}%)
**Margem de Contribuição Total Necessária:** R$ {mc_total:,.2f} ({mc_media_percentual*100:.1f}% do Faturamento)
""")
