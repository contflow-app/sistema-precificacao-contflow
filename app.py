...
for i in range(num_itens):
    ...
    if markup_divisor_ideal > 0 and custo_var_unit > 0 :
         ...
         # Calcular Markup sobre o Custo para o Preço Ideal
         if custo_var_unit > 0:
             markup_ideal_percentual = ((preco_ideal_calculado - custo_var_unit) / custo_var_unit) * 100
         else:
             markup_ideal_percentual = float('inf') # Custo zero, markup infinito
         ...
         item_dict["Margem Contribuição Ideal (R$)"] = contribuicao_ideal_rs # Adiciona MC ideal em R$
    ...
    with st.expander(f"Ver detalhes do cálculo para '{item_dict['Nome']}' (Preço Praticado R$ {preco_praticado:.2f})"):
    ...
    # --- Expander com Detalhes do Cálculo (Preço Ideal Sugerido) ---
    if preco_ideal_calculado > 0:
        with st.expander(f"Ver detalhes do cálculo para '{item_dict['Nome']}' (Preço Ideal Sugerido R$ {preco_ideal_calculado:.2f})"):
            st.markdown("**Receita Bruta (Preço Ideal Sugerido):**")
            st.write(f"R$ {preco_ideal_calculado:.2f}")

            st.markdown("**(-) Custos e Despesas Variáveis:**")
            st.write(f"- Custo Variável Unitário Direto: R$ {custo_var_unit:.2f}")
            st.write(f"- Impostos ({percentual_impostos*100:.1f}%): R$ {impostos_ideal:.2f}")
            st.write(f"- Taxas Variáveis ({percentual_taxas*100:.1f}%): R$ {taxas_ideal:.2f}")
            st.write(f"- Comissão ({percentual_comissao*100:.1f}%): R$ {comissao_ideal:.2f}")
            st.markdown(f"**= (=) Margem de Contribuição Unitária:** R$ {contribuicao_ideal_rs:.2f} **({contribuicao_ideal_perc:.1f}%)**")

            st.markdown("**Cálculo do Markup:**")
            if custo_var_unit > 0:
                 st.write(f"Markup sobre o Custo Variável Unitário: (Preço Ideal - Custo Variável) / Custo Variável")
                 st.write(f"({preco_ideal_calculado:.2f} - {custo_var_unit:.2f}) / {custo_var_unit:.2f} = {(preco_ideal_calculado - custo_var_unit)/custo_var_unit:.2f}")
                 st.write(f"**Markup Ideal:** {markup_ideal_percentual:.1f}%")
            else:
                 st.write("Markup Ideal: Infinito (Custo Variável Unitário é zero)")


            st.caption(f"""
            Este é o preço sugerido para que este item, ao ser vendido, gere uma Margem de Contribuição que contribua
            proporcionalmente para cobrir as Despesas Fixas totais, o Lucro Líquido Alvo e a Reserva/Reinvestimento desejados,
            considerando o faturamento previsto e os percentuais de impostos, taxas e comissões.
            """)
...
if items_data:
    ...
    st.dataframe(df_resultados.style
                 .format({
                     ...
                     "Markup Ideal (%)": "{:.1f}%" if df_resultados['Markup Ideal (%)'].max() != float('inf') else "{}" # Format infinito diferente
                 })
    ...
    # --- Novo Gráfico: Componentes do Preço ---
    st.subheader("Composição do Preço (R$)")

    # Prepare data for stacked bar chart
    df_components = pd.DataFrame(index=df_plot.index)
    df_components['Custo Variável (Praticado)'] = df_plot['Custo Variável Unitário (R$)']
    df_components['Impostos (Praticado)'] = df_plot['Impostos Praticado (R$)']
    df_components['Taxas (Praticado)'] = df_plot['Taxas Praticado (R$)']
    df_components['Comissão (Praticado)'] = df_plot['Comissão Praticado (R$)']
    df_components['MC (Praticado)'] = df_plot['Margem Contribuição Praticada (R$)']

    df_components['Custo Variável (Ideal)'] = df_plot['Custo Variável Unitário (R$)'] # Custo é o mesmo
    df_components['Impostos (Ideal)'] = df_plot['Impostos Ideal (R$)']
    df_components['Taxas (Ideal)'] = df_plot['Taxas Ideal (R$)']
    df_components['Comissão (Ideal)'] = df_plot['Comissão Ideal (R$)']
    df_components['MC (Ideal)'] = df_plot['Margem Contribuição Ideal (R$)']


    fig_comp, ax_comp = plt.subplots(figsize=(max(8, len(df_plot)*1.2), 6)) # Ajusta largura
    bar_width_comp = 0.4

    # Posições das barras agrupadas
    index_comp = np.arange(len(df_plot))
    bar_pos_praticado = index_comp - bar_width_comp/2
    bar_pos_ideal = index_comp + bar_width_comp/2

    # Cores para os componentes
    colors = {
        'Custo Variável': 'lightgrey',
        'Impostos': 'lightcoral',
        'Taxas': 'gold',
        'Comissão': 'mediumseagreen',
        'MC': 'skyblue'
    }

    # Plotar barras stacked para Preço Praticado
    bottom_praticado = np.zeros(len(df_plot))
    for component in ['Custo Variável', 'Impostos', 'Taxas', 'Comissão', 'MC']:
        values = df_components[f'{component} (Praticado)']
        ax_comp.bar(bar_pos_praticado, values, bar_width_comp, label=f'{component} (Praticado)', bottom=bottom_praticado, color=colors[component], edgecolor='grey')
        bottom_praticado += values

    # Plotar barras stacked para Preço Ideal
    bottom_ideal = np.zeros(len(df_plot))
    for component in ['Custo Variável', 'Impostos', 'Taxas', 'Comissão', 'MC']:
         # Use custo variável da coluna 'Custo Variável (Ideal)' que é a mesma, mas mantém a lógica
         values = df_components[f'{component} (Ideal)']
         ax_comp.bar(bar_pos_ideal, values, bar_width_comp, label=f'{component} (Ideal)', bottom=bottom_ideal, color=colors[component], edgecolor='grey', alpha=0.7) # Ideal slightly transparent
         bottom_ideal += values

    ax_comp.set_ylabel('Valor (R$)')
    ax_comp.set_title('Composição do Preço de Venda: Praticado vs. Ideal')
    ax_comp.set_xticks(index_comp)
    ax_comp.set_xticklabels(df_plot.index, rotation=45, ha="right")

    # Criar handles e labels únicos para a legenda, agrupando Praticado e Ideal para cada componente
    handles, labels = ax_comp.get_legend_handles_labels()
    unique_labels = ['Custo Variável (Praticado)', 'Impostos (Praticado)', 'Taxas (Praticado)', 'Comissão (Praticado)', 'MC (Praticado)',
                     'Custo Variável (Ideal)', 'Impostos (Ideal)', 'Taxas (Ideal)', 'Comissão (Ideal)', 'MC (Ideal)']
    # Ordenar para que Praticado e Ideal do mesmo componente fiquem juntos
    order = [labels.index(l) for l in unique_labels if l in labels]
    ax_comp.legend([handles[idx] for idx in order], [labels[idx] for idx in order], bbox_to_anchor=(1.05, 1), loc='upper left')
    ...
    # Gráfico de Preços (R$) - Mantido, mas o Composição do Preço é mais detalhado
    st.subheader("Comparativo de Preços (R$)")
    ...
    # Remover barra de custo aqui para não poluir, já está no gráfico de composição
    ...
    ax_p.set_title('Preços de Venda: Ideal vs. Praticado')
